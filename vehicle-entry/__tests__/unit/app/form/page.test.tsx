import {
  ...
  FormPlaceHolders,
  InputLabels,
} from "@/app/constant/form-constant";
...
describe("should render a form components", () => {
  ...
  it("should render all place holders in the form", () => {
    render(<EntryFormPage />);
    expect(
      screen.getByPlaceholderText(FormPlaceHolders.DRIVER_NAME)
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText(FormPlaceHolders.PHONE_NUMBER)
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText(FormPlaceHolders.PHONE_NUMBER)
    ).toBeInTheDocument();
  ...
  it("should display error messages when values is not provided", async () => {
    ...
    const driverNameError = await screen.findByText(
      FormValidationConstants.MISSING_DRIVER_NAME
    );
    const plateNumberError = await screen.findByText(
      FormValidationConstants.MISSING_VEHICLE_TYPE
    );
    const phoneNoError = await screen.findByText(
      FormValidationConstants.MISSING_PHONE_NO
    );
  ...
  it("should render a text for invalid value in the form component fields", async () => {
    ...
    fireEvent.change(
      screen.getByPlaceholderText(FormPlaceHolders.PLATE_NUMBER),
      {
